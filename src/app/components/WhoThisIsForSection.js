...
export default function WhoThisIsForSection({ data, whatsappPhone }) {
  ...
  return (
    <section id="who-this-is-for">
      {/* Main content — navy background with image accent */}
      <div className="relative bg-navy overflow-hidden py-14 lg:py-20">
          ...
          <div className="grid lg:grid-cols-12 gap-10 lg:gap-16 items-center">
            ...
            <div className="lg:col-span-5">
              ...
              <a
                ...
                className="inline-flex items-center gap-2.5 bg-gold hover:bg-gold-dark text-navy font-semibold px-6 py-3 rounded-lg text-sm transition-all duration-200 hover:shadow-lg hover:shadow-gold/20"
              >
                {data.ctaText}
            ...
            {/* Right — bullet cards in 2x2 grid */}
            <div className="lg:col-span-7">
              <div className="grid sm:grid-cols-2 gap-4">
                {data.bullets.map((bullet, i) => (
                  <div key={i} className="group flex gap-4 items-start p-5 rounded-xl bg-white/[0.06] backdrop-blur-sm border border-white/10 hover:bg-white/10 hover:border-gold/30 transition-all duration-300">
                    <div className="flex-shrink-0 w-10 h-10 rounded-lg bg-gold/15 flex items-center justify-center text-gold">
                      {bulletIcons[i]}
                    </div>
                    <p className="text-sm text-white/80 leading-relaxed pt-1.5">{bullet}</p>
                  </div>
                ))}
              </div>
            </div>
      ...
      {/* Stats bar — continuation of navy */}
        ...
        <div className="max-w-7xl mx-auto px-5 lg:px-8 py-10 lg:py-12">
          <div className="grid grid-cols-2 lg:grid-cols-4">
            {data.stats.map((stat, i) => (
              <div key={i} className={`py-3 lg:py-0 ${i < data.stats.length - 1 ? 'lg:border-r lg:border-white/10' : ''}`}>
